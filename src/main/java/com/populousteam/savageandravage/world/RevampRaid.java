...
import com.populousteam.savageandravage.init.SavageEffectRegistry;
...
public class RevampRaid extends Raid {
    ...
    private RevampRaid.Status status;
    private int field_221361_y;
    private Optional<BlockPos> field_221362_z = Optional.empty();

    public RevampRaid(int p_i50144_1_, ServerWorld p_i50144_2_, BlockPos p_i50144_3_) {
        super(p_i50144_1_, p_i50144_2_, p_i50144_3_);
        this.id = p_i50144_1_;
        ...
        this.status = RevampRaid.Status.ONGOING;
    }

    public RevampRaid(ServerWorld p_i50145_1_, CompoundNBT p_i50145_2_) {
        super(p_i50145_1_, p_i50145_2_);
        ...
        this.status = RevampRaid.Status.func_221275_b(p_i50145_2_.getString("Status"));
        this.heroes.clear();
    ...
    public boolean isStopped() {
        return this.status == RevampRaid.Status.STOPPED;
    }

    public boolean isVictory() {
        return this.status == RevampRaid.Status.VICTORY;
    }

    public boolean isLoss() {
        return this.status == RevampRaid.Status.LOSS;
    }
    ...
    public void increaseLevel(PlayerEntity p_221309_1_) {
        if (p_221309_1_.isPotionActive(SavageEffectRegistry.BADOMEN)) {
            this.badOmenLevel += p_221309_1_.getActivePotionEffect(SavageEffectRegistry.BADOMEN).getAmplifier() + 1;
            this.badOmenLevel = MathHelper.clamp(this.badOmenLevel, 0, this.getMaxLevel());
        }

        p_221309_1_.removePotionEffect(SavageEffectRegistry.BADOMEN);
    }

    public void stop() {
        this.active = false;
        this.bossInfo.removeAllPlayers();
        this.status = RevampRaid.Status.STOPPED;
    }

    public void tick() {
        if (!this.isStopped()) {
            if (this.status == RevampRaid.Status.ONGOING) {
                ...
                if (!this.world.func_217483_b_(this.center)) {
                    if (this.groupsSpawned > 0) {
                        this.status = RevampRaid.Status.LOSS;
                    } else {
                        this.stop();
                    }
                }
                ...
                if (this.func_221301_k() && !this.func_221289_z() && i == 0) {
                    if (this.postRaidTicks < 40) {
                        ...
                        this.status = RevampRaid.Status.VICTORY;
    ...
    private void spawnNextWave(BlockPos p_221294_1_) {
        ...
        for (RevampRaid.WaveMember raid$wavemember : RevampRaid.WaveMember.VALUES) {
    ...
    private void markDirty() {
        if (SavageAndRavageCore.instance.revampRaid != null) {
            SavageAndRavageCore.instance.revampRaid.markDirty();
        }
    }
    ...
    private int func_221330_a(RevampRaid.WaveMember p_221330_1_, int p_221330_2_, boolean p_221330_3_) {
    ...
    private int func_221335_a(RevampRaid.WaveMember p_221335_1_, Random p_221335_2_, int wave, DifficultyInstance p_221335_4_, boolean p_221335_5_) {
        ...
        return i > 0 ? p_221335_2_.nextInt(i) : 0;
    }
    ...
    enum Status {
        ...
        private static final RevampRaid.Status[] field_221278_e = values();

        private static RevampRaid.Status func_221275_b(String p_221275_0_) {
            for (RevampRaid.Status raid$status : field_221278_e) {
