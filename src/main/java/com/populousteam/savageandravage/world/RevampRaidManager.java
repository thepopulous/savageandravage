...
public class RevampRaidManager extends WorldSavedData {
    ...
    public RevampRaid func_215167_a(int p_215167_1_) {
        return this.byId.get(p_215167_1_);
    }

    public void tick() {
        ++this.tick;
        Iterator<RevampRaid> iterator = this.byId.values().iterator();

        while (iterator.hasNext()) {
            RevampRaid raid = iterator.next();
    ...
    public static boolean func_215165_a(AbstractRaiderEntity p_215165_0_, RevampRaid p_215165_1_) {
    ...
    @Nullable
    public RevampRaid badOmenTick(ServerPlayerEntity p_215170_1_) {
        if (p_215170_1_.isSpectator()) {
            ...
            if (dimensiontype == DimensionType.THE_NETHER) {
                ...
                RevampRaid raid = this.findOrCreateRaid(p_215170_1_.getServerWorld(), blockpos1);
                boolean flag = false;
                if (!raid.func_221301_k()) {
                    ...
                    p_215170_1_.removePotionEffect(SavageEffectRegistry.BADOMEN);
    ...
    private RevampRaid findOrCreateRaid(ServerWorld p_215168_1_, BlockPos p_215168_2_) {
        RevampRaid raid = (RevampRaid) SavageAndRavageCore.instance.findRaid(p_215168_2_);
    ...
    public void read(CompoundNBT nbt) {
        ...
        ListNBT listnbt = nbt.getList("RevampRaids", 10);

        for (int i = 0; i < listnbt.size(); ++i) {
            CompoundNBT compoundnbt = listnbt.getCompound(i);
            RevampRaid raid = new RevampRaid(this.world, compoundnbt);
            this.byId.put(raid.getId(), raid);
        }
    ...
    public CompoundNBT write(CompoundNBT compound) {
        ...
        for (RevampRaid raid : this.byId.values()) {
            CompoundNBT compoundnbt = new CompoundNBT();
            raid.write(compoundnbt);
            listnbt.add(compoundnbt);
        }

        compound.put("RevampRaids", listnbt);
        return compound;
    }

    public static String func_215172_a(Dimension p_215172_0_) {
        return "revampraids" + p_215172_0_.getType().getSuffix();
    }
    ...
    @Nullable
    public RevampRaid findRaid(BlockPos p_215174_1_, int distance) {
        RevampRaid raid = null;
        double d0 = (double) distance;

        for (RevampRaid raid1 : this.byId.values()) {
