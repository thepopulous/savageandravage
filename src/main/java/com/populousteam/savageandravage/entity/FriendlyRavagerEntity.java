...
import com.populousteam.savageandravage.message.MessageRavagerAttackStat;
import com.populousteam.savageandravage.message.MessageRavagerDushStat;
import com.populousteam.savageandravage.message.MessageRavagerStopDushStat;
            ...
            if (this.isBoosting()) {
                ...
                for (LivingEntity livingentity : this.world.getEntitiesWithinAABB(LivingEntity.class, this.getBoundingBox().grow(0.75D, 0.0D, 0.75D))) {
                    if (livingentity != this && (this.getControllingPassenger() == null || this.getControllingPassenger() != null && livingentity != this.getControllingPassenger()) && !this.isOnSameTeam(livingentity) && (!(livingentity instanceof ArmorStandEntity) || !((ArmorStandEntity) livingentity).hasMarker()) && this.getDistanceSq(livingentity) < 26.0D) {
                        livingentity.attackEntityFrom(DamageSource.causeMobDamage(this), (int) this.getAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getValue() * 0.6F);
                        livingentity.knockBack(this, f1 * 0.7F, MathHelper.sin(this.rotationYaw * ((float) Math.PI / 180F)), -MathHelper.cos(this.rotationYaw * ((float) Math.PI / 180F)));
                    }
                }
            }
        }
        ...
        if (this.isAlive()) {
            ...
            if (this.isBeingRidden() && this.canBeSteered()) {
                ...
                if (this.canPassengerSteer()) {
                    ...
                    super.travel(new Vec3d(strafe, vector.y, getAIMoveSpeed() * forward * 2.0F));
