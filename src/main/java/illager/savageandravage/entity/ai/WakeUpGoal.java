...
public class WakeUpGoal extends Goal {
    ...
    public boolean shouldExecute() {
        if (this.illager.world.isDaytime()) {
            ...
            return this.illager.isSleeping() && illager.posY < (double) this.illager.getBedPosition().get().getY() + 0.4D || this.illager.isSleeping() && !this.illager.getBedPosition().get().withinDistance(illager.getPositionVec(), 1.14D);
        }
    }
