...
public class SavageEntityRegistry extends ForgeRegistryEntry<SavageEntityRegistry> {
    ...
    public static void addEntitySpawn() {
        ForgeRegistries.BIOMES.getValues().stream().forEach((biome -> {
            if (biome.getSpawns(EntityClassification.MONSTER).contains(EntityType.SKELETON)) {
                biome.getSpawns(EntityClassification.MONSTER).add(new Biome.SpawnListEntry(SKELETONVILLAGER, 10, 1, 2));
            }
        }));
    }
}